...
/player-info  → formatted response using async region detection (NEW)
/get          → raw protobuf JSON with auto region detection (NEW)
...
# ── Import Message from protobuf to fix NameError ──
...
try:
    ...
    _MAJOR_BYTES = (
        ...
        b'external_sdcard_avail_storage\x18\x42 \x01(\x05\x12%\n\x1dexternal_sdcard_total'
        b'_storage\x18\x43 \x01(\x05\x12\x10\n\x08login_by\x18I \x01(\x05\x12\x14\n\x0clibrary'
...
REGION_LANG = {
    "ME":"ar","IND":"hi","ID":"id","VN":"vi","BD":"bn","PK":"ur","TW":"zh","SAC":"es","BR":"pt"
}
ALL_REGIONS = list(REGION_LANG.keys())
SUPPORTED_REGIONS = {"IND","BD","SAC","NA","SG","ID","TW","VN","TH","ME","PK","BR","EUROPE"}
...
# ── Global caches ──────────────────────────────────────────────────────────────
_token_cache: Dict[str, dict] = {}          # ffinfoo region tokens (legacy)
_uid_region_cache: Dict[str, str] = {}      # uid → region (legacy)
...
# ══════════════════════════════════════════════════════════════════════════════
#  FFINFOO — REAL PROTOBUF PLAYER INFO SYSTEM  (src type) — LEGACY
# ══════════════════════════════════════════════════════════════════════════════

# ─── SERVICE ACCOUNTS (replace with your own working Free Fire credentials) ───
_SERVICE_ACCOUNTS = {
    "IND": {"uid":"4218389302", "password":"NILAY-9LRRJQ7P3-NR-CODEX"},
    "BD":  {"uid":"6549243316", "password":"Shihabkaksk_B44XB_xSaeed_EG9U1"},
    "ID":  {"uid":"7579018789", "password":"shskskjsjs_YHSUZ_xSaeed_SK9FI"},
    "TW":  {"uid":"7579042218", "password":"sjaksjjshd_CGO75_xSaeed_0DX9T"},
    "VN":  {"uid":"7579086477", "password":"sjksksjsjdhd_0ZMIO_xSaeed_6XMFU"},
    "ME":  {"uid":"YOUR_UID_HERE", "password":"YOUR_PASSWORD_HERE"},
    "PK":  {"uid":"YOUR_UID_HERE", "password":"YOUR_PASSWORD_HERE"},
    "SG":  {"uid":"7578959983", "password":"ahihab_ZVZ8I_xSaeed_S0D0A"},
...
# ─── LEGACY OAuth + MajorLogin (used by old endpoints) ───────────────────────
def _get_oauth_token(uid: str, password: str) -> Optional[str]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning(f"OAuth status {resp.status_code} for uid={uid}")
            return None
        data = resp.json()
        open_id = data.get("open_id", "")
        token = data.get("access_token", "")
        if not token:
            logger.warning(f"No access_token in response for uid={uid}")
            return None
        ...
        payload2 = aes_cbc_encrypt_main(login_req.SerializeToString())
        login_headers = {
        ...
        lr = requests.post(
            ...
            data=payload2, headers=login_headers, timeout=15, verify=False
        )
...
def _ensure_region_token(region: str) -> Optional[Tuple[str, str]]:
    ...
    result = generate_jwt_sync(acc["uid"], acc["password"], region.upper())
    if not result.get("success"):
        logger.warning(f"JWT generation failed for region {region}: {result.get('error')}")
        return None, None
...
def get_player_info_src(uid: str) -> Dict:
    ...
    if cached_region:
        ...
        if result:
            return result
    for region in ["IND","SG","ID","BR","VN","TH","ME","PK","BD","US","RU","TW"]:
...
def major_login_sulav(access_token: str, open_id: str, region: str) -> Optional[Dict]:
    ...
    url = "https://loginbp.common.ggbluefox.com/MajorLogin" if use_bluefox else "https://loginbp.ggpolarbear.com//MajorLogin"
    headers = {
        "Accept-Encoding": "gzip",
        "Authorization": "Bearer",
        "Connection": "Keep-Alive",
        "Content-Type": "application/x-www-form-urlencoded",
        "Host": "loginbp.common.ggbluefox.com" if use_bluefox else "loginbp.ggpolarbear.com",
        "ReleaseVersion": "OB54",
...
# ══════════════════════════════════════════════════════════════════════════════
#  NEW ASYNC TOKEN SYSTEM (from simplified API) — for /get and /player-info
# ══════════════════════════════════════════════════════════════════════════════

_async_token_cache = TTLCache(maxsize=50, ttl=28800)  # 8 hours
_region_cache = TTLCache(maxsize=200, ttl=3600)       # 1 hour per UID
...
async def _get_access_token_async(account: str) -> Tuple[str, str]:
...
async def _create_jwt(region: str):
    account = _get_account_credentials(region)
    token_val, open_id = await _get_access_token_async(account)
    body = json.dumps({"open_id": open_id, "open_id_type": "4", "login_token": token_val, "orign_platform_type": "4"})
    proto_bytes = await _json_to_proto(body, FreeFire_pb2.LoginReq())
    payload = aes_cbc_encrypt_main(proto_bytes)
...
async def _initialize_tokens():
    tasks = [_create_jwt(r) for r in SUPPORTED_REGIONS]
    await asyncio.gather(*tasks)

async def _refresh_tokens_periodically():
    while True:
        await asyncio.sleep(25200)
        await _initialize_tokens()

async def _get_token_info(region: str) -> Tuple[str, str, str]:
    info = _async_token_cache.get(region)
    if info and time.time() < info['expires_at']:
        return info['token'], info['region'], info['server_url']
    await _create_jwt(region)
    info = _async_token_cache[region]
    return info['token'], info['region'], info['server_url']

async def _GetAccountInformation(uid, unk, region, endpoint):
    region = region.upper()
    if region not in SUPPORTED_REGIONS:
        raise ValueError(f"Unsupported region: {region}")
    payload = await _json_to_proto(json.dumps({'a': uid, 'b': unk}), main_pb2.GetPlayerPersonalShow())
    data_enc = aes_cbc_encrypt_main(payload)
    token, lock, server = await _get_token_info(region)
    headers = {
        'User-Agent': USERAGENT,
        'Connection': "Keep-Alive",
        'Accept-Encoding': "gzip",
...
async def _fetch_region_from_uid(uid: str) -> str:
    uid = str(uid)
    if uid in _region_cache:
        return _region_cache[uid]
    for region in SUPPORTED_REGIONS:
        try:
            data = await _GetAccountInformation(uid, "7", region, "/GetPlayerPersonalShow")
            if data and data.get("basicInfo", {}).get("nickname"):
                _region_cache[uid] = region
                return region
        except Exception:
            continue
    raise ValueError(f"Could not determine region for UID {uid}")
...
async def _startup_async():
    await _initialize_tokens()
    asyncio.create_task(_refresh_tokens_periodically())
...
def index():
    ...
    endpoints = {
        # ── NEW async endpoints ──
        "/player-info":    {"method":"GET","params":{"uid":"Player UID"},"example":f"{base}/player-info?uid=3074306062","desc":"Formatted player info with auto region detection (NEW)"},
        "/get":            {"method":"GET","params":{"uid":"Player UID"},"example":f"{base}/get?uid=3074306062","desc":"Raw protobuf JSON with auto region detection (NEW)"},
        # ── Player Info (legacy) ──
        "/level":          {"method":"GET","params":{"uid":"Player UID"},"example":f"{base}/level?uid=3074306062","desc":"Player level + XP progress"},
        "/region":         {"method":"GET","params":{"uid":"Player UID"},"example":f"{base}/region?uid=3074306062","desc":"Detect player region (legacy)"},
        ...
        "/refresh":        {"method":"GET","desc":"Refresh all region tokens (legacy + async)","example":f"{base}/refresh"},
        # ── Images ──
    ...
    return jsonify({
        ...
        "version": "7.1",
        "author": "Merged (ffinfoo + sulavcodex + async endpoints)",
...
def route_player_info_new():
    """Formatted player info with auto region detection (uses new async token system)."""
    uid = request.args.get('uid')
    if not uid:
        return jsonify({"error": "Please provide UID."}), 400
    try:
        region = asyncio.run(_fetch_region_from_uid(uid))
        if region not in SUPPORTED_REGIONS:
            return jsonify({"error": f"Unsupported region: {region}"}), 400
        raw_data = asyncio.run(_GetAccountInformation(uid, "7", region, "/GetPlayerPersonalShow"))
        formatted = _format_response(raw_data)
        return jsonify(formatted), 200
    except ValueError as ve:
        return jsonify({"error": str(ve)}), 404
    except Exception as e:
        logger.exception("Error in /player-info")
        return jsonify({"error": f"Failed to fetch account info: {str(e)}"}), 500

@app.route('/get', methods=['GET'])
def route_get_raw():
    """Raw protobuf JSON with auto region detection (uses new async token system)."""
    uid = request.args.get('uid')
    if not uid:
        return jsonify({"error": "Please provide UID."}), 400
    ...
    if uid in _region_cache:
        try:
            region = _region_cache[uid]
            data = asyncio.run(_GetAccountInformation(uid, "7", region, "/GetPlayerPersonalShow"))
            return Response(
...
# ─── REFRESH endpoint (extended to also refresh async tokens) ──────────────
...
def route_refresh():
    ...
    # Start async refresh
    try:
        asyncio.run(_initialize_tokens())
        async_refreshed = True
    except Exception as e:
        logger.warning(f"Async token refresh failed: {e}")
        async_refreshed = False
    return jsonify({
        "message": "All token caches cleared. Async tokens refreshed.",
        "regions": list(_SERVICE_ACCOUNTS.keys()),
        "async_refreshed": async_refreshed
    })

# ─── Legacy /player-info endpoint (kept for backward compatibility) ──────────
# We already replaced it with the new one above. If you want to keep both,
# rename this to /player-info-legacy, but we'll keep the new one as /player-info.

# ─── Legacy endpoints (level, guild, etc.) remain unchanged ──────────────────
...
def route_version():
    return jsonify({
        "version":"7.1",
        "name":"FF Ultimate API",
        "merged":"ffinfoo+sulavcodex+async",
        "info_type":"protobuf-src",
        "features":["base64","custom-banner","custom-outfit","ai","real-player-protobuf","async-auto-region"],
    })
...
def route_analytics():
    return jsonify({
        ...
        "async_cached_regions": len(_async_token_cache),
        "async_cached_uids": len(_region_cache),
    })
...
def route_update_bio():
    ...
    raw_nickname = d.get("nickname", "")
    nickname = decode_nickname(raw_nickname) if raw_nickname else None   # <-- DECODED
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION: BIND INFO, PLATFORM CHECK & ACCOUNT SECURITY
# ═══════════════════════════════════════════════════════════════════════════════
...
def route_decode_jwt():
    ...
    nickname = decode_nickname(raw_nickname) if raw_nickname else None
    return jsonify({
...
if __name__ == "__main__":
    import warnings
    warnings.filterwarnings("ignore")
    # Start async token background tasks
    try:
        asyncio.run(_startup_async())
        logger.info("Async token system started.")
    except Exception as e:
        logger.warning(f"Async token startup warning: {e}")
