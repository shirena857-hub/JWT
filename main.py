...
def fetch_player_info(uid: str):
    if not uid:
        return None
    player_info_url = f"https://sb-ful-info.vercel.app/player-info?uid={uid}"
    try:
...
def outfit_image():
    ...
    # ---- Weapon IDs (from AccountInfo.EquippedWeapon) ----
    equipped_weapon_list = player_data.get("AccountInfo", {}).get("EquippedWeapon", []) or []
    weapon_skin_id = equipped_weapon_list[0] if len(equipped_weapon_list) >= 1 else None
    weapon_anim_id  = equipped_weapon_list[1] if len(equipped_weapon_list) >= 2 else None

    def fetch_weapon_image(weapon_id):
        if not weapon_id:
            return None
    ...
    futures.append(executor.submit(fetch_weapon_image, weapon_skin_id))
    futures.append(executor.submit(fetch_weapon_image, weapon_anim_id))
    ...
    positions = [
        ...
        {'x': 179, 'y': 603, 'height': 150, 'width': 150},  # ← weapon skin (adjust)
        {'x': 99, 'y': 791, 'height': 150, 'width': 150},  # ← weapon animation (adjust)
    ]
